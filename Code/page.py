...
def KNN():
    ...
    val_scortest = []
    for k in range(1, nb):
        score1 = cross_val_score(KNeighborsClassifier(k), x_train, y_train, cv=int(p2.lineEdit_4.text()),
...
def NaveBais():
    ...
    val_scortest = []
    for k in range(1, nb):
        score1 = cross_val_score(GaussianNB(var_smoothing=k), x_train, y_train, cv=int(p2.lineEdit_4.text()),
                                 scoring='accuracy').mean()
        val_scortrain.append(score1)
        score2 = cross_val_score(GaussianNB(var_smoothing=k), x_test, y_test, cv=int(p2.lineEdit_4.text()),
                                 scoring='accuracy').mean()
        val_scortest.append(score2)
    # and the first axes using subplot populated with data
    xi = np.arange(1, nb)
...
def ArbeDis():
    ...
    val_scortest = []
    for k in range(1, nb):
        score1 = cross_val_score(DecisionTreeClassifier(criterion='gini', max_depth=k, random_state=0), x_train,
                                 y_train, cv=int(p2.lineEdit_4.text()), scoring='accuracy').mean()
        val_scortrain.append(score1)
        score2 = cross_val_score(DecisionTreeClassifier(criterion='gini', max_depth=k, random_state=0), x_test, y_test,
                                 cv=int(p2.lineEdit_4.text()), scoring='accuracy').mean()
        val_scortest.append(score2)
    # and the first axes using subplot populated with data
    xi = np.arange(1, nb)
    p2.figure.clear()
...
def reseau():
    ...
    val_scortest = []
    for k in range(1, nb):
        score1 = cross_val_score(MLPClassifier(max_iter=k * 10), x_train, y_train, cv=int(p2.lineEdit_4.text()),
                                 scoring='accuracy').mean()
        val_scortrain.append(score1)
        score2 = cross_val_score(MLPClassifier(max_iter=k * 10), x_test, y_test, cv=int(p2.lineEdit_4.text()),
                                 scoring='accuracy').mean()
        val_scortest.append(score2)
    # and the first axes using subplot populated with data
    xi = np.arange(1, nb)
